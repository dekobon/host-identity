...
Sources live under `crates/host-identity/src/sources/`. A new source
should:

- Implement the `Source` trait.
- Get a `SourceKind` variant and an `ids::source_ids` constant.
- Handle the sentinel, empty-file, and missing-file cases explicitly
  (see `sources/util.rs::normalize`).
- Include tests covering: happy path, missing input, sentinel
  rejection, malformed input.
- Be wired into the appropriate `*_default_chain` function if it
  belongs in the default chain.

### Cloud and network sources

- Gate behind a per-provider Cargo feature.
- Stay generic over `HttpTransport` — don't depend on a concrete HTTP
  client.
- Enforce a bounded response size and a timeout via the transport.
- Include a mock-server test (see existing AWS/GCP tests) that
  exercises the request shape.
