...
`resolve()` or `resolve_with_transport(t)` — the ordering of sources,
what each source returns in each scenario, which outcomes advance the
chain and which short-circuit it, and how the raw identifier becomes
the final UUID.
